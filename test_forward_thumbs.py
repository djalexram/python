...
class TestForwardThumbs(object):
	def test_forward_thumb_down(self,selenium,proxy,server,preroll_ads):
		try:
			driver = selenium
			time.sleep(3)
			...
			assert sel.check_behavior(update_qstrings,"behavior[thumbs_down]") == "1", "Could not find request with behavior[thumbs_down] = 1"
	...
	def test_forward_thumb_up(self,selenium,proxy,server,preroll_ads):
		try:
			driver = selenium
			time.sleep(3)
			player = Player(driver,timeout,preroll_ads)
			...
			if len(next_list) > 0:
				...
				assert sel.check_for_dup_recs(playlist,next_list) == False, "Next call contained duplicate recs from initial watch call"
			lastUpdate=update_qstrings[len(update_qstrings)-1]
			assert sel.check_behavior(update_qstrings,"behavior[thumbs_up]") == "1", "Could not find request with behavior[thumbs_down] = 1"
			assert len(watch_calls) == expected_watch, "Extra watch calls: " + str(len(watch_calls)-1)
			update_min = 2
			maxi = 7
			maxi = sel.update_maxi(maxi)
